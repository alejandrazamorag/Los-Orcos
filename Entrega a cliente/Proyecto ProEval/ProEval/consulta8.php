...
if(isset($_POST['proyectosE']))
{
	$q=$conexion->real_escape_string($_POST['proyectosE']);
...
if ($buscarProyectos->num_rows > 0)
{
	...
	while($filaProyectos= $buscarProyectos->fetch_assoc())
	{
		...
		'<tr>
			...
			<td> <a href="Visualizar_Proyecto_Usuario_Est.php?txtnc='.$filaProyectos['idProyecto'].' && txtNombre='.$filaProyectos['Nombre'].' && txtdescripcion='.$filaProyectos['Descripcion'].' && txtfechaC='.$filaProyectos['Fecha_Creacion'].' && txtfechaL='.$filaProyectos['Fecha_Limite'].'"> Ver Estimaciones</a> </td>
		 </tr>
		';
	}
